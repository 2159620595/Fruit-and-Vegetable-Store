...
-- 1. 更新用户表，添加余额字段（检查是否存在）
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS 
     WHERE TABLE_SCHEMA = DATABASE() 
     AND TABLE_NAME = 'users' 
     AND COLUMN_NAME = 'balance') = 0,
    'ALTER TABLE users ADD COLUMN balance DECIMAL(10,2) DEFAULT 0.00 COMMENT ''用户余额''',
    'SELECT ''balance字段已存在'' as message'
));
...
SET @sql = (SELECT IF(
    ...
    'SELECT ''membership_level字段已存在'' as message'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = (SELECT IF(
    ...
    'SELECT ''total_recharge字段已存在'' as message'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
-- 8. 更新现有用户的会员等级（如果membership_level字段存在）
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS 
     WHERE TABLE_SCHEMA = DATABASE() 
     AND TABLE_NAME = 'users' 
    ...
    'SELECT ''membership_level字段不存在，跳过更新'' as message'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- 9. 创建触发器：充值成功后自动更新用户余额（如果不存在）
...
SET @sql = (SELECT IF(
    ...
    'SELECT ''idx_users_balance索引已存在'' as message'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = (SELECT IF(
    ...
    'SELECT ''idx_users_membership_level索引已存在'' as message'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = (SELECT IF(
    ...
    'SELECT ''idx_balance_transactions_user_type索引已存在'' as message'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
-- 14. 插入测试数据（可选）
-- INSERT IGNORE INTO users (username, email, balance, membership_level) VALUES 
-- ('test_user', 'test@example.com', 1000.00, '黄金会员');
