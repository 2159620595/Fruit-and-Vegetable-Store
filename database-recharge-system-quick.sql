-- 充值系统数据库表结构 - 快速版
-- 创建时间: 2024-01-20
-- 修复了order_stats字段问题，使用更简单的会员等级设置
...
BEGIN
    IF NEW.payment_status = 'success' AND OLD.payment_status != 'success' THEN
        ...
        SET membership_level = CASE 
            WHEN total_recharge >= 5000 THEN '钻石会员'
            WHEN total_recharge >= 2000 THEN '黄金会员'
            WHEN total_recharge >= 500 THEN '白银会员'
            ELSE '普通会员'
        END
        WHERE id = NEW.user_id;
    END IF;
